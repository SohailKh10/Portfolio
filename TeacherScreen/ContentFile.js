...
const ContentFile = ({ navigation, route }) => {
    ...
    const pickFile = async () => {
        try {
            ...
            formData.append("mainfolderfile", { uri: res[0].uri, name: res[0].name, type: res[0].type });
            formData.append("checklistitemid", global.checklistid);

            formData.append("FileName", "abc");
            formData.append("mainid", global.MainId);
            ...
            //save file to api
            var uri =
                global.API + 'Teacher/UploadMainFolderFiles';
            console.log(uri);
    ...
    const GetMainFolderFiles = () => {
        ...
        //  console.log(item.)
        var uri =
            global.API + `Teacher/GetMainFolderFiles?teacherId=${global.Id}&courseid=${global.Courseid}&sessionId=${global.SessionId}&semester=${global.Semester}&programId=${global.ProgramId}&checklistid=${global.Id}`
        console.log(uri);
        fetch(uri)
            .then(response => response.json())
            .then(json => {
