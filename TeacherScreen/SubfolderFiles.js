...
const SubfolderFiles = ({ navigation, route }) => {
    ...
    const AllowedSubFolderChecklistTeacherSide = () => {
        let item = route.params.detail;
        //  console.log(item.)
        var uri =
            global.API + 'Teacher/AllowedSubFolderChecklistTeacherSide?subfolderid=' + global.Id;
        console.log(uri);
        fetch(uri)
    ...
    useState(() => {
        ...
        AllowedSubFolderChecklistTeacherSide();
    }, [SubFolder]);
    ...
    const OnSubFolderCheck = (item) => {
        ...
        console.log(item);
        var uri =
            global.API + `Teacher/OnSubFolderCheckTeacherside?subfolderid=${global.Id}&subfolderchecklistid=${item.ID}`
        console.log(uri);
        fetch(uri)
            .then(response => response.json())
            .then(json => {
            ...
            <FlatList
                ...
                renderItem={({ item, index, separators }) => (
                    <View style={{ flexDirection: 'row', alignItems: 'center', padding: 20, paddingHorizontal: 10, paddingTop: 10, }}>
                        <Text style={styles.textStyle}>{item.Name}</Text>
                        <CheckBox value={item.Flag == "1" ? true : false} onValueChange={() => OnSubFolderCheck(item)}
                        >
                        </CheckBox>
                    </View>
                )}
