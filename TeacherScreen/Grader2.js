...
const Grader2 = ({ navigation, route }) => {
    ...
    // const options = [
    //     { label: 'Fall 2022', value: 'option1' },
    //     { label: 'Spring 2021', value: 'option2' },
    //     { label: 'Fall 2023', value: 'option3' },
    // ];
    ...
    const OnSubFolderCheck = (item) => {
        ...
        console.log(item);
        var uri =
            global.API + 'Teacher/UpdateItemPermission?id=' + item.ID;
        console.log(uri);
        fetch(uri)
            .then(response => response.json())
            .then(json => {
    ...
    const Item = ({ item }) => {
        ...
        return (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', padding: 20, paddingHorizontal: 10, paddingTop: 10, }}>
                ...
                <CheckBox value={item.cstatus} onValueChange={() => OnSubFolderCheck(item)}
                />
